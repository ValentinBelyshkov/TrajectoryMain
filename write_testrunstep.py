content = '''import { useState, type RefObject, useEffect } from "react";
...
export function TestRunStep({ 
  ...
  useEffect(() => {
    ...
    if (isTestRunning && countdown !== null && countdown > 0) {
      timer = setTimeout(() => setCountdown(countdown - 1), 1000);
    } else if (isTestRunning && countdown === 0) {
      setCountdown(null);
    }
    return () => {
      if (timer) clearTimeout(timer);
    };
  }, [isTestRunning, countdown]);
          ...
          >
            {isTestRunning ? (
              <>
                ...
                <span>Выполняется инициализация... {countdown !== null ? `(осталось ${countdown} сек)` : ""}</span>
              </>
            ) : (
              <>
                <Play className="w-6 h-6" />
                <span>Запустить тест (15 сек)</span>
              </>
            )}
          </Button>
...
with open('/opt/main/Trajectory/TWA/client/components/calibration/TestRunStep.tsx', 'w') as f:
    f.write(content)
print('TestRunStep.tsx written')
