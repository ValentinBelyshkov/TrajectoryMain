...
async def _gateway_action(component: str, action: str, value: Optional[str] = None) -> list[str]:
    ...
    async with _gateway_client() as client:
        ...
        r = await client.post(f"/api/v1/components/{component}/{action}", json=payload)
        r.raise_for_status()
        data = r.json()
...
async def control_terraslam_component(action: ComponentAction, request: Request) -> CommandResponse:
    ...
    if action.component == "all" and action.action in ("start", "restart", "stop", "kill"):
        ...
        for comp in target_components:
            try:
                ...
                if comp.startswith("publisher_") and action.action in ("start", "restart"):
                    if action.project_id:
                        if comp == "publisher_realsense":
                            frames_dir = f"{SLAM_DB}/projects/{action.project_id}/procframe"
                        else:
                ...
                # realsense publisher persists captured frames into procframe
                start_value = None
                if comp == "publisher_realsense":
                    start_value = f"{SLAM_DB}/projects/{action.project_id}/procframe" if action.project_id else None
                ...
                if action.action == "restart":
                    ...
                    results = await _gateway_action(comp, action.action, value=start_value)
                    combined_output += "\n".join(results) + "\n"
            except Exception as e:
    ...
    if gateway_component.startswith("publisher_") and action.action in ("start", "restart"):
        ...
        if gateway_component == "publisher_realsense":
            frames_dir = f"{SLAM_DB}/projects/{action.project_id}/procframe"
        else:
            frames_dir = f"{SLAM_DB}/projects/{action.project_id}/frames"
        os.makedirs(frames_dir, exist_ok=True)
    ...
    try:
        if action.action == "restart":
            ...
            start_res = await _gateway_action(gateway_component, "start", value=start_value)
            output = "\n".join(kill_res + start_res)
        else:
            results = await _gateway_action(gateway_component, action.action, value=start_value)
            output = "\n".join(results)
