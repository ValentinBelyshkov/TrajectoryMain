...
cv::Mat FrameDrawer::DrawFrame(float imageScale)
{
    ...
    if ( currentFrame.mnId >= 0) // убедитесь, что ID корректен
    {
...
std::cout << "imageScale = " << imageScale << std::endl;
		...
		std:cout<<vpLocalMap.size()<<std::endl;
    ...
    std::string imgfilename = "frame_" + std::to_string(currentFrame.mnId) + ".jpg";
    cv::imwrite(imgfilename, im);
    cv::Mat imWithInfo;
