...
cv::Mat FrameDrawer::DrawFrame(float imageScale)
{
    ...
    float thDepth;
	std::cerr << "TEST CALL FOR FRAMEFUN: " << std::endl;
    Frame currentFrame;
    ...
    if (shouldSave && currentFrame.mnId >= 0) // убедитесь, что ID корректен
    {
        std::stringstream ss;
        ss << "frame_" << currentFrame.mnId << ".txt";
        ...
        if (keypointFile.is_open())
        {
            for(size_t i = 0; i < currentFrame.N; ++i)
            {
                ...
                if(pMP && !pMP->isBad())
                {
                    cv::KeyPoint kp = currentFrame.mvKeysUn[i];
                    cv::Mat worldPos = pMP->GetWorldPos(); // 3x1 Mat

                    // Убедитесь, что worldPos — это float и имеет 3 элемента
                    if (worldPos.type() == CV_32F && worldPos.total() == 3)
                    {
                        float u = kp.pt.x;
                        float v = kp.pt.y;
                        float x = worldPos.at<float>(0);
                        float y = worldPos.at<float>(1);
                        float z = worldPos.at<float>(2);

                        keypointFile << u << " " << v << " " << x << " " << y << " " << z << "\n";
                    }
                }
            }
            keypointFile.close();
    ...
    cv::Point org(10, 30);               // (x, y) — координаты начала текста
    int fontFace = cv::FONT_HERSHEY_SIMPLEX;
    double fontScale = 1.0;
    cv::Scalar color(255, 255, 255);     // Белый цвет в BGR
    int thickness = 2;
